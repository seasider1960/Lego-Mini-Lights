...
bool handleFileRead(String path) { // send the right file to the client (if it exists)
  ...
  if (path.endsWith("/")) path += "MiniInst.html";          // If a folder is requested, send the index file
  ...
  switch (type) {
    ...
    case WStype_TEXT:                     // if new text data is received
      ...
      if (payload[0] == '#') {            // we get RGB data
        ...
        uint32_t t = (uint32_t) strtol((const char *) &payload[1], NULL, 16);   // decode time data
        int tz =          t & 0x3FF;
        Serial.println(tz);
        if ((tz > 15) && (tz < 19)){
          timeZone = tz-6;
